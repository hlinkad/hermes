...
LLM_PROVIDERS = ["openrouter", "openai", "ollama", "deepseek"]
...
with st.sidebar:
    ...
    source_paths = st.text_input("Source paths", value=defaults.source_paths, help="Comma-separated extracted source roots, e.g. /gdrive/hermes-brain")
    book_paths = st.text_input("Book paths", value=defaults.book_paths, help="Comma-separated book files/directories. Supports PDFs via pypdf and text/markdown with page markers.")
    book_pages_per_chunk = st.number_input("Book pages per chunk", value=defaults.book_pages_per_chunk, min_value=1, max_value=10)
    ...
    with col1:
        index_clicked = st.button("Index", type="primary", disabled=not (vault_path or source_paths or book_paths), use_container_width=True)
    with col2:
        clear_index_clicked = st.button("Clear Index", use_container_width=True)
    ...
    if index_clicked:
        config = get_settings(
            ...
            book_pages_per_chunk=book_pages_per_chunk,
...
def render_sources(sources, vault_path):
    ...
    for i, source in enumerate(sources, 1):
        ...
        label = source.citation or source.file_name
        with st.expander(f"{i}. {label} {score_str}"):
